...
const linkApi = {
  ...
  PREPROD: {
    ...
    GetInfo: 'https://preprod-api-linkage.rmb-lab.jp/linkapi/GetInfo',
    AuthorizationCode: 'Bearer zRgYGhbWcRnnaBGE27aSGK+tN_Lwn9a',
    redirectUri:
      'https://stg.grp03.id.rakuten.co.jp/rms/nid/login?service_id=rm005&client_id=rmn_app_web&redirect_uri=https%3A%2F%2Fnetwork.mobile.rakuten.co.jp%2Fsupport%2Ftest-rakuten-link-form%2F00d1s0000000q35%2F%3Fl-id%3Dshort_mno_support_rakuten-link-form&err_redirect_uri=https%3A%2F%2Fnetwork.mobile.rakuten.co.jp%2F&scope=memberinfo_read_safebulk%2Cmemberinfo_read_point%2Cmemberinfo_get_card_token%2C30days%40Access%2C90days%40Refresh&contact_info_required=false&rae_service_id=rm005',
  },
}
...
const SupportRakutenLinkForm: NextPage = () => {
  ...
  useEffect(() => {
    ...
    if (authCode) {
      const fetchData = async (authCode: string) => {
        try {
          const url = new URL(linkApi.PREPROD.GetInfo)
  ...
  const handleDisplayForm = (event: { preventDefault: () => void }) => {
    ...
    reset({
      ...
      token: userInfoData?.token ? userInfoData.token : 'unknown',
      agree: false,
    })
  }
