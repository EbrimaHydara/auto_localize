...
const simulationBtnCpn = document.querySelector('.js-simulation-btn-cpn');
...
askElms.forEach((askElm, askElmIdx) => {
  ...
  switch (step) {
    ...
    case 3: {
      ...
      radioElms.forEach(radioElm => {
        radioElm.addEventListener('click', () => {
          simulationBtnCpn.scrollIntoView({behavior: 'smooth', block: 'center'});
        });
      });
      break;
    }
...
(async () => {
  ...
  console.log(encodeURIComponent('https://network.mobile.rakuten.co.jp/campaign/fee-simulation/result/?l-id=campaign_fee-simulation_campaign_fee-simulation-result'))

  const simulationBtnNormal = document.querySelector('.js-simulation-btn-normal');
  // シミュレーションボタン選択時
  simulationBtnCpn.addEventListener('click', e => {
    e.preventDefault();

    sessionStorage.setItem('simulationCpnInput', 'visited'); // add session storage so as to check the visit on the cpn result page

    const hrefBase = 'https://network.mobile.rakuten.co.jp/campaign/fee-simulation/result/?l-id=campaign_fee-simulation_campaign_fee-simulation-result';
    let hrefMod = hrefBase;
    ...
    const mailmagazinePermissionChecked = document.getElementById('js-mailmagazine-permission-checkbox-01').checked;

    hrefMod += `${paramPrefix}data=${size}`;
    if (carrier) {
      hrefMod += `&carrier=${carrier}`;
    }
    if (familyPlan) {
      hrefMod += `&family=${familyPlan}`;
    }
    if (mailmagazinePermissionChecked) {
      hrefMod += '&ekeyflag=true';
    }
    const hrefEncoded = encodeURIComponent(hrefMod);
    const pattern = /redirect_uri=([^&]+)/;
    const match = simulationBtnCpn.href.match(pattern);
    if (match) {
      const matchedString = match[1];
      simulationBtnCpn.href = simulationBtnCpn.href.replace(matchedString, hrefEncoded);
    }
    location.href = simulationBtnCpn.href;
  });

  simulationBtnNormal.addEventListener('click', e => {
    e.preventDefault();
    const activeTab = getActiveTab();
    ...
    let paramPrefix = '?';
    // クエリに既に「?」があるとき
    if (simulationBtnNormal.href.match(/\?/)) {
      paramPrefix = '&';
    }
    ...
    simulationBtnNormal.href += `${paramPrefix}data=${size}`;
    if (carrier) {
      simulationBtnNormal.href += `&carrier=${carrier}`;
    }
    if (familyPlan) {
      simulationBtnNormal.href += `&family=${familyPlan}`;
    }
    location.href = simulationBtnNormal.href;
  });
