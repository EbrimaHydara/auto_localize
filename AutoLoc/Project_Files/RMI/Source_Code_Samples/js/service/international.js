...
const dispInfo = document.getElementById('js-International-info');

if (dispInfo) {
    $.ajax({
        url: '/assets/json/service-international-info.jsonp',
        dataType: 'jsonp',
        ...
        if (data['information'].length > 0) {
            for (let i = 0, len = data['information'].length; i < len; i++) {
                listHtml += [
                    ...
                    '<p class="c-Attention_Body">',
                    data['information'][i].text + '<a href="' + data['information'][i].link + '">' + data['information'][i].text_link_area + '</a>',
                    '</p>',
                    '</div>',
                    '</div>'
                ].join('');
