  ...
  <li class="c-Card_Normal">
    ...
    <figure>
      <a :href="`${path.sitepath}product/accessory/${detailDir}/?l-id=product_accessory_${detailDir}`">
        <img :src="`${path.sitepath}assets/img/accessory/${detailDir}/pht-accessory-00.png?${summary.updateimg}`"
              :alt="summary.product">
      </a>
    </figure>
    ...
    <div class="accessory-apple-top-Card_Price">
      <p>
        <span v-if="summary.price_old"><span class="c-Txt_Alphanumeric-m accessory-apple-top-Card_Price-borderdouble">{{ summary.price_old | addComma }}</span>円
        <span v-if="isShowPriceComma(detailDir)" class="c-Txt_Cap"> ※{{ findIndex }}</span>→</span>
...
export default {
  ...
  computed: {
    ...
    findIndex() {
        const product = this.summary.product;
        if (this.type === 'case') {
            if (product.indexOf('iPhone 14') !== -1) {
            　  return 1;
            } else if (product.indexOf('iPhone 13') !== -1) {
                return 2;
            }
        } else {
