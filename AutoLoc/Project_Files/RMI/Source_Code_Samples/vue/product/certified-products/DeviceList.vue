<template>
    <div>
        ...
        <div class="u-Adjust_Mt-40">
            <p class="c-Txt_Cap service-Search_Notice">※記載の「確認時のソフトウェアバージョン」以降が楽天モバイルに対応します。</p>
            <template v-if="this.deviceList.length === 0">
                <ResultNon />
            </template>
            <template v-else>
                <ResultSp :deviceList="deviceList" />
                <ResultPc :deviceList="deviceList" />
            </template>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        async getJson() {
            const res = await axios.get('/assets/json/certified-products-list.json');
        ...
        setJson() {
            ...
            jsonData
                .then(response => {
                    ...
                    this.defaultDeviceList = sortedData;
                    this.deviceList = sortedData;
        ...
        searchReset() {
            ...
            this.deviceList = this.defaultDeviceList;
            this.keywordValue = '';
        ...
        filterdData(e) {
            ...
            this.deviceList = this.keywordValue && this.manufacturerValue && this.manufacturerValue !== 'all' ? // キーワード & メーカーで検索
                                this.defaultDeviceList
                                    .filter(item => {
                                ...
                                this.defaultDeviceList.filter(item => {
                                    return item.oem === this.manufacturerValue;
                                });
        },
