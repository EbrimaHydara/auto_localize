<template>
    <div>
        <div v-show="filtered.length > 0">
            <p class="c-Txt_Emp-01 u-Adjust_Mt-16">該当するショップが、{{ filtered.length }}件見つかりました。</p>
        ...
        <div v-show="filtered.length === 0"
            class="shop-Shoplist_Noresult">
            <p class="c-Txt_Emp-01">該当するショップが見つかりませんでした。</p>
            <p class="u-Adjust_Mt-16">条件を変更して、検索ください。</p>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        filtered () {
            ...
            let city = mutations.getCity();

            if (mutations.getPrefecture()) {
                filteredData = filteredData.filter((shop) => {
                    return shop.location.prefecture === mutations.getPrefecture();
                });
            }
            ...
            filteredData = this.judgeShowPref(filteredData.length, city, filteredData);
    ...
    methods: {
        ...
        judgeShowPref (len, city, data) {
            if (this.type === 'gadget' && len !== 0 && len < 3 && city !== '') {
                let filteredData = this.shops;
