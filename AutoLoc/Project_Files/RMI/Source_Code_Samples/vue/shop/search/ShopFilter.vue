<template>
    <div>
        ...
        <div v-show="!checkNoAvailableService" class="shop-Search_Filters">
            <div class="c-Data">
                <a @click="restoreFilters"
                    href="#modal-search"
                    class="c-Data_Filter js-Modal" style="white-space:nowrap;">
                    <span class="c-Icon_Sliders-l c-Data_Icon" style="text-decoration: none;color: #ff008c;"></span><span>対応サービス・店舗条件で絞り込む</span>
                </a>
            </div>
            <div v-show="selectedServicesLabel"
                class="shop-Search_Filters-label">絞り込み条件：{{ selectedServicesLabel }}</div>
        </div>
        <div id="modal-search" style="display:none;">
            ...
            <button @click="setFilter"
                        type="button"
                        class="c-Btn_Secondly-large js-Modal_Close">検索する</button>
            </div>
        </div>
    </div>
</template>
