...
class HTMLFileLocalizer(FileLocalizer):
    def __init__(self, project_id):
        ...
        self.translatable_tags = ['title', 'div', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'a', 'li', 'td', 'th', 'button', 'label', 'option', 'img']
        self.formatting_tags = ['span', 'br', 'b', 'i', 'small', 'strong', 'em', 'u', 'mark', 'sub', 'sup']
    ...
    def localize_html_files(self):
        ...
        # Get the localized html files for report
    ...
    def _mark_translatable_strings(self, soup):
        ...
        for tag in soup.find_all(self.translatable_tags):
            ...
            elif tag.string or any(child.name in self.formatting_tags for child in tag.find_all(recursive=False)):
    ...
    def _extract_and_save_strings(self, soup, html_file):
        ...
        for tag in soup.find_all(attrs={"data-i18n": True}):
            key = tag['data-i18n']
            source_json[key] = ''.join(str(e) for e in tag.contents)
        
        for tag in soup.find_all(attrs={"data-i18n-alt": True}):
            key = tag['data-i18n-alt']
            source_json[key] = tag['alt']
