...
def localize_and_extract(file_content, file_path):
    combined_prompt = f"""
    Please analyze the given Vue.js (.vue) file content and perform the following tasks:
    1. Mark all translatable strings using the `t` function or `$t` method, ensuring to replace them with human-readable keys in the format ('{file_path}:key'), where 'key' is a suggested key representation. This includes strings in the <template> (HTML), <script> (JavaScript), and <style> (CSS) sections.
    2. Extract all marked strings along with their corresponding keys into a JSON object in the format {{"key": "value"}}.
    3. Ensure the code remains intact and unbroken during localization, preserving the original structure, logic, and styling of the Vue component.
...
if __name__ == "__main__":
    target_folder = 'files_to_loc/vue'  # Change this to your target folder path
    output_folder = 'loced_files/vue'  # Change this to your desired output folder path
