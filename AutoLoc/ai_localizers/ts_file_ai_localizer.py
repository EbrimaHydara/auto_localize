...
def localize_and_extract(file_content, file_path):
    combined_prompt = f"""
    Please analyze the given TypeScript (TS) file content and perform the following tasks:
    1. Mark all translatable strings using the `t` function, ensuring to replace them with human-readable keys in the format ('{file_path}:key'), where 'key' is a suggested key representation. Ensure the localization covers all translatable strings in the TypeScript file, including those in strings, comments, and user-facing messages.
    2. Extract all marked strings along with their corresponding keys into a JSON object in the format {{"key": "value"}}.
    3. Ensure the code remains intact and unbroken during localization, preserving the original structure, type definitions, and functionality of the TS file.
...
if __name__ == "__main__":
    target_folder = 'files_to_loc/ts'  # Change this to your target folder path
    output_folder = 'loced_files/ts'  # Change this to your desired output folder path
